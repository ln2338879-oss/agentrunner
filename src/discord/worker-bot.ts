...
export function createWorkerBot(input: {
  ...
  client.once("ready", () => {
    const channelNote = input.channelId ? `listening on channel ${input.channelId}` : "message intake disabled";
    console.log(`[${input.role}] logged in as ${client.user?.tag ?? "unknown"}; ${channelNote}`);
  });

  client.on("messageCreate", async (message) => {
    if (!shouldProcessWorkerMessage({ configuredChannelId: input.channelId, messageChannelId: message.channelId, authorIsBot: message.author.bot })) return;
    try {
      await handleWorkerMessage({ message, role: input.role, orchestrator: input.orchestrator, store: input.store, config: input.config });
    } catch (error) {
...
async function handleWorkerMessage(input: {
  ...
  if (steer) {
    input.store.recordSteeringMessage({ id: `STEER-${Date.now()}`, taskId: steer.taskId, discordMessageId: input.message.id, content: steer.content });
  ...
  if (isCommand(input.message.content) && !parseRetryCommand(input.message.content)) {
    const commandResult = await handleDirectorCommand({ content: input.message.content, store: input.store });
    if (commandResult) {
  ...
  const contentWithAttachments = withAttachmentContext(input.message.content, attachmentContext.markdown);
  ...
  if (retryTaskId) {
    ...
    const result = await input.orchestrator.handleUserRequest({
      content: withSessionContext(forceRoleContent(input.role, retryContent(retryTaskId, task.title)), input.store, session.id),
      discordMessageId: input.message.id,
      discordChannelId: input.message.channelId,
    });
    await input.message.reply(formatUserTaskResponse({ result, prefix: `${roleDisplayName(input.role)} retry ${result.taskId}` }));
    return;
  }
  ...
  await input.message.reply(formatUserTaskResponse({ result, prefix: `${roleDisplayName(input.role)} result ${result.taskId}` }));
}
