...
type OfficeZone = { id: string; label: string; x: number; y: number; w: number; h: number };
type OfficeAgent = {
  id: string;
  name: string;
  role: string;
...
type OfficeSnapshot = {
  ...
  links: { status: string; tasks: string; health: string };
};
...
export function handleDashboardRequest(request: Request, store: RuntimeStore): Response {
  ...
  if (url.pathname === "/health") return json({ ok: true, service: "agentrunner-dashboard" });
  if (url.pathname === "/api/status") return json(store.getDashboardStatus());
  if (url.pathname === "/api/office/snapshot") return json(buildOfficeSnapshot(store));
  if (url.pathname === "/api/office/events") return officeEventsStream(store);
  ...
  if (url.pathname === "/") {
    return new Response(renderDashboardHtml(store), { headers: { "content-type": "text/html; charset=utf-8" } });
  }

  return json({ error: "Not found" }, 404);
}

function buildOfficeSnapshot(store: RuntimeStore): OfficeSnapshot {
  const status = store.getDashboardStatus();
  const tasks = store.listRecentTasks(30);
  const roles = Array.from(new Set([...DEFAULT_ROLES, ...status.byRole.map((row) => row.role), ...tasks.map((task) => task.assignedTo)])).filter(Boolean);
  ...
  return {
    ...
    links: { status: "/api/status", tasks: "/api/tasks?limit=30", health: "/health" },
  };
}
